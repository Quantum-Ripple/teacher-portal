        ...
        <tbody>
          <tr v-for="record in records" :key="record.id">
            ...
            <td class="border px-2 py-1">
              <select v-model="record.status" class="border px-2 py-1">
  ...
  methods: {
    ...
    async loadSession() {
      try {
        ...
        // Fetch student details from studentApi for this class & stream
        const students = await studentApi.filter(this.classLevel, this.stream);

        // Map session records to include student names & admission numbers
    ...
    async saveAttendance() {
      try {
        ...
        const payload = {
          class_level: this.classLevel, // required by backend
          stream: this.stream,          // required by backend
        ...
        console.log("PATCH response:", res); // Debug
...
// import custom icons (same as MarkAttendance.vue)
...
export default {
  data() {
    return {
      ...
      // same icon buttons as MarkAttendance
      statusOptions: [
